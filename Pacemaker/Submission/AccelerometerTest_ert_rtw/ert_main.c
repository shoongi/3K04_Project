...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(AccelerometerTest_M) == (NULL)) &&
    !rtmGetStopRequested(AccelerometerTest_M);
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(AccelerometerTest_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(AccelerometerTest_M, true);
      }

      if (rtmGetStopRequested(AccelerometerTest_M) == true) {
        rtmSetErrorStatus(AccelerometerTest_M, "Simulation finished");
        break;
      }
    }

    AccelerometerTest_step();

    /* Get model outputs here */
    rtExtModeCheckEndTrigger();
    stopRequested = !((rtmGetErrorStatus(AccelerometerTest_M) == (NULL)) &&
                      !rtmGetStopRequested(AccelerometerTest_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(AccelerometerTest_M, "stopping the model");
  runModel = 0;
}
...
void *backgroundTask(void *arg)
{
  while (runModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(AccelerometerTest_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(AccelerometerTest_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(AccelerometerTest_M, 0);
  ...
  /* Initialize model */
  AccelerometerTest_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(AccelerometerTest_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(AccelerometerTest_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(AccelerometerTest_M, true);
    }
  }
